...
#define NOMSPEED 1500
#define FASTSPEED 2000
#define ERRORBAND 100
...
float Kp = 1.0, Kd = 0.5;   // PD control gains
//float Ki = 0.0;   // I control gain
//int Eint = 0;               // integral (sum) of control error
int e_prev = 0;               // previous error (for D control)
...
//                DRV_USART_Write(appData.usartHandle, appData.readBuffer, appData.readLength);
                ...
                for(ii=0; ii<appData.readLength; ii++) {
                    if (appData.readBuffer[ii] == '\n' || appData.readBuffer[ii] == '\r'){
                        ...
                        int u;
                        if(qq > ERRORBAND){   //GO RIGHT
                            LATBbits.LATB7 = 0;
                            LATBbits.LATB9 = 0;
//                            EPint = EPint + qq;
                            
                            // position control signal:
//                            u = Kp * qq + Ki * EPint + Kd * (qq - e_prev);
                            u = Kp*qq + Kd*(qq - e_prev);
                            if (u > NOMSPEED) {
                                u = NOMSPEED;
                            } else if (u < 0) {
                                u = 0;
                            ...
                            OC1RS = (unsigned int) (NOMSPEED + u);  //LW++
//                            OC1RS = (unsigned int) (FASTSPEED);  //LW++
                            OC2RS = (unsigned int) (NOMSPEED - u);  //RW-- 
                        }
                        else if(qq < -ERRORBAND){   //GO LEFT
                            LATBbits.LATB7 = 0;
                            LATBbits.LATB9 = 0;
                            
                            qq = -qq;   //to make the error a positive int
//                            EPint = EPint + qq;
...
//                            u = Kp * qq + Ki * EPint + Kd * (qq - e_prev);
                            ...
                            OC1RS = (unsigned int) (NOMSPEED - u);  //LW--
                            OC2RS = (unsigned int) (NOMSPEED + u);  //RW++
//                            OC2RS = (unsigned int) (FASTSPEED);  //RW++
                        }
                        else if(qq == 0){   //NO COM FOUND -- STOP or REVERSE
                            LATBbits.LATB7 = 1;
                            LATBbits.LATB9 = 1;
                            OC1RS = 1000;
                            OC2RS = 1000;   
